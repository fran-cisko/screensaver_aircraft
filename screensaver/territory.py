...
# TODO falta hacer la parte sur.

    def update_position(self, target_flying_object: FlyingObject):
        other_than_flying_objects = [f for f in self.flying_objects if f != target_flying_object]
        for other_object in other_than_flying_objects:
            if target_flying_object.is_colliding_with(other_object):
                self.flying_objects.remove(target_flying_object)
